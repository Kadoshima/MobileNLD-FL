...
float getBatteryCurrent() {
    ...
    uint16_t discharge_raw = M5.Power.Axp192.readRegister13(0x7C);
    float discharge_current = discharge_raw * 0.5;  // LSB = 0.5mA
    
    // 充電電流レジスタも読む (0x7A-0x7B)
    uint16_t charge_raw = M5.Power.Axp192.readRegister12(0x7A);
...
void setup() {
    ...
    Serial.println("BLE Adaptive HAR - Flash Power Logger (Fixed Current Reading)");
...
void logPowerToFlash() {
    ...
    // Get power readings with fixed current reading
    float vbat = M5.Power.getBatteryVoltage() / 1000.0;  // Convert to V
    float ibat = getBatteryCurrent();  // 修正版の電流読み取り
