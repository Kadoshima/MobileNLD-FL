...
// 固定値による電流推定（M5StickC Plus2用）
#define CURRENT_QUIET 25.0      // mA (IDLE/QUIET状態)
#define CURRENT_UNCERTAIN 30.0  // mA (UNCERTAIN状態)
#define CURRENT_ACTIVE 40.0     // mA (ACTIVE状態)
...
void setup() {
    ...
    Serial.println("=== BLE Adaptive HAR - M5StickC Plus2 Edition ===");
    Serial.println("Note: Using estimated current values (No AXP192)");
    ...
    if (LittleFS.exists(LOG_FILE_PATH)) {
        Serial.println("Warning: Previous log exists. Will append.");
    }
    ...
    if (statusFile) {
        ...
        statusFile.printf("Device: M5StickC Plus2 (No AXP192)\n");
        statusFile.printf("Current estimation: Q=%d,U=%d,A=%d mA\n", 
                         (int)CURRENT_QUIET, (int)CURRENT_UNCERTAIN, (int)CURRENT_ACTIVE);
        statusFile.close();
    }
...
void dumpDataAndClear() {
    ...
    if (logFile) {
        ...
        Serial.println("\nSend 'DELETE' to clear log file, or 'KEEP' to preserve");
...
void logPowerToFlash() {
    ...
    // Get power readings (Plus2: 電流は推定値)
    float vbat = M5.Power.getBatteryVoltage() / 1000.0;  // Convert to V
    float ibat = getEstimatedCurrent();  // 推定電流値
    float power = vbat * ibat;  // mW
...
void loop() {
    ...
    if (M5.Power.getBatteryLevel() < 10) {
        ...
        // Plus2用のシャットダウン
        digitalWrite(HOLD_PIN, LOW);  // 電源を切る
    }
